  ...
  [
    new RegExp(
      [
        ...
        `(?<![数何])(?:${KNUM}|${K1}|十)\\s*(?:グラム|g(?![a-z])|ミリ(?![もの])|ml|cc|リットル|秒(?!でも)|パーセント|°|クリック|段(?!階的|と|落|構)|倍|時間|センチ)`,
        // 十度 / 三度 / 九十二度 (一度 = "once" stays)
        `(?<![数何])(?:${KNUM}|[二三四五六七八九]|十)\\s*度(?!目|だけ|に|も|と)`,
        `(?<![数何十])${K1}分(?!の|け|か|野|類|解|量)`,
        `(?<![数何])${KNUM}分`,
        `${K1}?十?${K1}?分半`,
        // a kanji ratio / time: 一対十五 / 一：十五
        `${KNUM}\\s*[対比:：/]\\s*(?:${KNUM}|${K1}|十)|(?:${K1}|十)\\s*[対比:：/]\\s*${KNUM}`,
        // a bare two-digit or hundreds kanji number: 九十二くらい / 湯を二百 (十分に / 数十秒 stay)
        `(?<![数何十百])${KNUM}(?![分人日年月回つ本種代均屋貨姓])`,
      ].join("|"),
      "u"
    ),
