...
| **反映状況** | **2026-09-14 時点: 未反映**。trigger は 2026-05-12 版のまま。このファイルと `docs/strategy.md` の変更が main に入ったあとで貼り替え、ここを「YYYY-MM-DD 反映済み」に書き換える |
...
- **2026-09-14 ジャンル実験層を追加**（正本: sns-hub `docs/strategy/genre-experiment.md` / 写し: このリポ `docs/strategy.md` 冒頭の「ジャンル実験」節）。2026-05-12 版の trigger 指示文からの差分は次のとおりで、それ以外は 2026-05-12 版のまま:
  1. 手順 0.5 の末尾に「ジャンル実験」節の読み込み（method 最適化より優先）を追加
  2. 手順 1 の先頭に `0) ジャンル試行の状態` を追加（集計コマンドの実行・前回モードの引き継ぎ・判定日と遅延判定・節が無い場合の退避動作）
  3. 手順 1 の a) c) d) を IG / YT 別・型の初回投稿日 F 以降・アカウントごとの順位づけに変更し、配信死亡モードの例外を追加
  4. 手順 1 の e) f) に構造実験の提案（レポートに書くだけ・当日の原稿や JSON に反映しない）と、レポート冒頭の「ジャンル試行の状態」「ジャンル判定」「構造実験の提案」節を追加
  5. 手順 2 に配信死亡モードの例外（80/20 を使わない場合の選び方・凍結の解除）を追加し、手順 7 に「提案を JSON に反映しない」注意を追加
...
**ジャンル実験（日次 PDCA の上位層・最優先）**: `docs/strategy.md` 冒頭の「ジャンル実験」節も必ず読む。このリポの IG / YT 2 アカウントの試行台帳（試行 #・開始日 S・型の初回投稿日 F・導入時の判定）、判定窓の計算と集計コマンド、閾値、モードの決め方、通常時と配信死亡モード中の日次 PDCA のルール、レポート節のフォーマットが書いてある。**この節の指示は、手順 1〜2 の method 最適化より優先する**。IG と YT は別アカウントとして別々に評価し、数字を合算しない。
...
- 前回モードは `docs/pdca/` の `YYYY-MM-DD.md` のうち今日より前の日付で最新のものから読む (無ければ台帳の「導入時の判定」)。同節の「モードの決め方」でアカウントごとに今日のモード (通常 / 切替候補 / 配信死亡モード) を決める。**モードが変わるのは判定日と遅延判定のときだけ** (判定値は毎日出すが、判定日以外は前回モードのまま。配信死亡の域に落ちたら「注意」行を書く)
- 判定日 (または遅延判定) なら、続行 / 切替候補 / 配信死亡を「ジャンル判定」節に書く
- ここで決めたモードが、下の c)〜f) と手順 2 の振る舞いを決める。結果は f) のレポート冒頭に書く
...
**a) 過去パフォーマンスを読む**
- `data/performance-history.json` から過去 14 日の `stats.views` (YT) / `instagram.views`・`instagram.saved` (IG。`instagram` か `instagram.views` が null の回は IG 側から除外) / `stats.likes` / `title` / `discovery.method` を抽出 (これはネタ・method 分析用の直近 14 日で、0) の判定窓とは別。method 比較は型の初回投稿日 F 以降の回だけで行う)
...
- TOP 3 method と WORST 3 method を**アカウントごとに**特定 (合算スコアを作らない)
- **配信死亡モードの例外** (0) で決めたモード): 配信死亡モードのアカウントの数字は method 比較・凍結に使わない。2 アカウントとも配信死亡モードならテーブルは参考表示に留め、TOP/WORST method を決めない。片方だけなら、生きている側のアカウントの TOP/WORST だけを使う

**e) 今日の改善アクションを 3 つまで決める** (戦略のコンテンツ柱比率、勝ち筋 method 継続 or 新 method 試行を考慮。**配信死亡モードのアカウントについては method のアクションを書かず、構造実験 (タイトル個別化 / 型変更 / ジャンル変更) を「何を変えるか / 何で測るか / 14 日後の合格ライン」で提案する** (同じ動画を IG と YT に出しているので、動画そのものを変える提案は `IG / YT 共通` の 1 行にまとめてよい)。構造実験の提案と次ジャンル候補は**レポートに書くだけ**で、当日の原稿・タイトル・JSON には反映しない。Web 調査に時間を使わず短く書く。このリポの制約: 販促 (豆の購入への導線) は維持し、候補は豆の購入に繋がる型に限る)

**f) `docs/pdca/$TODAY.md` にレポート** (**冒頭 (タイトル直下) に「ジャンル試行の状態」節** (`docs/strategy.md`「ジャンル実験」節のフォーマットどおり) / 判定日・遅延判定のときだけ「ジャンル判定」節 / 配信死亡モードのアカウントがある日は「構造実験の提案」節 / 続けて TOP3・WORST3 (IG / YT 別) / Method別テーブル / 直近14日のトピック / 気づき / 今日のAction / Method提案 (2 アカウントとも配信死亡モードの日は「配信死亡モードのため省略」と 1 行) / 戦略更新提案)
...
- 2 アカウントとも配信死亡モード → 80/20 を使わない。method は性能データで選ばず、下の選定制約 (曜日の型・直近 2 日と同じ method を連続で選ばない・NG) を満たす method のうち**直近 14 日の使用回数が少ないもの**を選ぶ (同数なら `docs/strategy.md` の Discovery Methods 表の上から)。WORST method の凍結はすべて解除して扱う
- 片方だけ配信死亡モード → 生きている側のアカウントの TOP 3 から 80/20 で選ぶ
...
**注意 (ジャンル実験)**: 手順 1 の「構造実験の提案」「次ジャンル候補」はレポートに書くだけ。この JSON の形式・必須フィールド・当日の型には反映しない。
