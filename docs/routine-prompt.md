...
6.5. **核の変化（`core`・24 字以内）と動き（`motion`）**: 「なぜ変わる？」の場面の主役。`core` はこの回で伝えたい一番大事な変化を 1 行で（例「温度を下げると、苦味の成分が溶けにくくなる」）。`motion` はそれを絵でどう動かすか: `type` は `liquid`（器の中のコーヒーの色が変わる）/ `meter`（味のメーターが動く）/ `compare`（2 杯の色が離れていく）/ `dissolve`（粉から成分が溶け出す）、`shade` は `{ "from", "to" }`（コーヒーの濃さ、0〜100 の整数）、`meters` は 1〜3 個の `{ "label", "from", "to" }`（`label` は 4 字以内の味の名前、`from` / `to` は 0〜100 の整数）。**下書き（今日の回の常備原稿）の `core` と `motion` をそのまま使ってよい**。どちらも無いと検証 NG
