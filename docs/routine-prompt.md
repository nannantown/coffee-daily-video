...
11. **ナレーション（任意）**: 書かなければテンプレートが使われる（最後に「次回は〜」が自動で入る）。書くなら `title` / `numbers` / `visual` / `steps` / `taste` / `tips` の 6 項目の合計を **180 字以内**を目安に（締めの `cta` は書かない — 次回予告つきの締め約 50 字が自動で足され、全体で 230 字前後になる。書くと次回予告が消える）
